...
$(document).ready( function() {
	...
	$("#filterDropbox").droppable({
		drop: function(event, ui) {
			...
			if($(this).find("div#"+fieldId).length == 0){
				var url = contextURL + "/ext/reports/customReportRenderFilter.action";
				var dropBoxDiv = this;
				$.ajax({
					type : "POST",
					url: url,
	...
	$("#orderPriorityDropBox").droppable({
		drop: function(event, ui) {
			...
			if(isFieldThere == 0){
				var url = contextURL + "/ext/reports/customReportRenderColumn.action";
				$.ajax({
					type : "POST",
					url: url,
					data : {fieldId : fieldId},
	...
	$('#generateReport').off('click').on('click',function () {
	   	...
	   	var url = contextURL + "/ext/reports/runReport.action";
   ...
   $("#BankPP").off('click').on('click',function () {
	   if(!$(this).is(':checked')) {
		   $.ajax({
		   		type: "POST",
	            url: contextURL+'/ext/reports/resetBanks.action',
	            dataType: 'html',
	            processdata: true,
	            success: function(data) {
   ...
   $("#BusPP").off('click').on('click',function () {
	   if(!$(this).is(':checked')){
		   $.ajax({
		   		type: "POST",
	            url: contextURL+'/ext/reports/resetSites.action',
	            dataType: 'html',
	            processdata: true,
	            success: function(data) {
   	...
   	$(".nav-template").off('click').on('click', function(){
		...
		if(validateAdhoc()){
			...
			$(indicator).show();
			var data = $("#adhocReportForm").serialize();
			var url = contextURL + "/ext/reports/customReportSaveTemplate.action";
			$.ajax({
				type : "POST",
				url: url,
				data : data,
...
function treeData(indicator, instrumentTypes, fieldName) {
	$(indicator).show();
	var url = contextURL + "/ext/reports/customReportSearchFields.action";
	$.ajax({
		type : "POST",
		url: url,

	...
	$(document).ready(function() {
        ...
        $('body').off('click', '#generateReport').on('click', '#generateReport', function(e) {
			...
			$("#fxRateDivId").removeClass("show").addClass("hide");
 			...
 			if(validateIssuance()){
        		...
        		var url = contextURL +"/ext/reports/recordCountForIssuance.action";
     			$.ajax({
     				type: "POST",
     				url: url,
     				dataType: 'json',
        ...
        $('body').off('click', '#exportResult').on('click', '#exportResult', function(e) {
			...
			e.preventDefault();
			assignValues();  
			var url = contextURL+'/ext/reports/exportIssuanceExpirationReportResults.action?fileName='+$('#headerText').text();

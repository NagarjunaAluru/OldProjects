	...
	$(document).ready(function() {
		...
		$('body').off('click', '#generateReport').on('click', '#generateReport', function(e) { 
			 ...
			 if(validateFeesPaid()){
        		 ...
        		 var url = contextURL +"/ext/reports/recordCountForFeesPaid.action";
     			 $.ajax({
     				type: "POST",
     				url: url,
     				dataType: 'json',
		...
		$('body').off('click', '#exportResult').on('click', '#exportResult', function(e) {
			...
			e.preventDefault();
        	var url = contextURL+"/ext/reports/exportFeesPaidReportResults.action?fileName="+$('#headerText').text();
        	assignValues();
        ...
        if($('#siteSelection').is(':empty')){
        	$.ajax({
        		type: "POST",
	            url: contextURL+'/ext/reports/addSiteSelection.action',
	            dataType: 'html',
	            processdata: true,
	            success: function(data) {
	            	$("#siteSelection").empty().append(data);
        ...
        if($('#bankSelection').is(':empty')){
        	$.ajax({
        		type: "POST",
	            url: contextURL+'/ext/reports/addBankSelection.action',
	            dataType: 'html',
	            processdata: true,
	            success: function(data) {
	            	$("#bankSelection").empty().append(data);

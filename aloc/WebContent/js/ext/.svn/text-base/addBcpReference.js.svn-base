...
$(document).ready(function() {
	...
	$("a#addadditionalBUContactRef").off('click').on('click', function() {
		...
		if(buContactPersonFieldAddIndex <2){
			...
			var url = contextURL+'/ext/requestor/addAdditionalBcpField.action?buContactPersonFieldAddIndex='+(buContactPersonFieldAddIndex+1);
	...
	$('a.deleteBCP-ce').off('click').on('click', function(e) {
		...
		var url = contextURL+'/ext/requestor/removeAdditionalField.action';

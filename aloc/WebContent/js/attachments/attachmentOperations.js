		...
		$('.alocFileupload').fileupload(
				{
			        ...
			        method: 'POST',
			        	...
			        	if(varifyResponse(data.result)) {
			        		...
			        		if(renderArea) {
			        			renderArea.html(contentToRender);
			        		}				        		
...
//			add new row for attachments
			$('.add-attachment').each(function() {			
				...
				var url = contextURL+'/int/addAttachment.action?newAttachmentIndex=' + newAdditionalAttachmentIndex+'&atmtTypeId='+atmtTypeId+'&instrumentTypeId='+instrumentTypeId;			
...
function prepareHTMLForFile(responseData) {	
	...
	var downloadURL = contextURL + "/int/atmt/download.action?geLibFileId=" + responseData.attachment.geFileId + "&typeId=" + responseData.typeId + "&formatId=" + responseData.formatId;
 ...
 * 
 */
function deleteAttachment(geLibFileId,typeId,index,evt) {	
...
var url = contextURL +"/int/atmt/delete.action?";
...
function openTextareaSection(){	
	...
	var url = contextURL +"/int/atmt/closureDelete.action?";
...
//permissions to check for bank or surety
function checkAtmtPermissions(indexId){	
    var val =$("#bankOrSuretyAtmtPermission"+indexId).is(':checked'); 
    var instrumentTypeId = $('#atmtInstrumentTypeId').val();
    if(val != undefined && val == true){
    	n = window.open('#atmtPermissionsConfirmModal', '_self');
		$('#permissionsIndexId', n.document).val(indexId);				  		
		if(instrumentTypeId=='3' || instrumentTypeId=='6')
		{			
			$("#bankPermissionsTestId").hide();   
			$("#suretyPermissionsTestId").show();
		}else
		{		
			$("#bankPermissionsTestId").show();   
			$("#suretyPermissionsTestId").hide();
		}
		$('#atmtPermissionsConfirmModal').modal({	 	 
	        show: 'true'	        
		});				
	}	
}

function atmtPermissionsConfim(){	
	var indexId = $('#permissionsIndexId').val();	
	var val = $("#bankOrSuretyAtmtPermission"+indexId).is(':checked');		
	if(val != undefined && val == true){
		$("#bankOrSuretyAtmtPermission"+indexId).attr("checked",false);     	
	}	
}
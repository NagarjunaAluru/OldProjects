...
<link href="${pageContext.request.contextPath}/ext/public/css/common/pagenavi.css" rel="stylesheet" type="text/css">
<div class="form-mod">
	...
	<s:if test="%{#lookUpNo != null}">
		...
		<s:url action="BusinessContactPersonLookup" namespace="/ext" var="getBusinessContactPersonURL" escapeAmp="false" encode="true">
	...
	<div class="row" style="margin-bottom: 0px;">
			...
			<div class="">
				<c:choose>
					<c:when test="${param.pageOrigin eq 'Report'}">
						<p><a class="btn-tertiary" href="javascript:;" onClick="javascript:returnToMainPage();">&#60; Return to Report</a></p>
					</c:when>
					<c:otherwise>
						<p><a class="btn-tertiary" href="javascript:;" onClick="javascript:returnToMainPage();">&#60; Return to Request Form</a></p>
					</c:otherwise>
				</c:choose>
			</div>
		</div>
		<div class="span4a right">
			<div class="form-row">
				<s:form id="businessContactPersonLookupForm">
					<c:choose>
						<c:when test="${param.requestorType eq 'currentRequestor'}">
							<p style="margin-bottom:5px;">Requestor</p>
						</c:when>
						<c:otherwise>
							<p style="margin-bottom:5px;">Contact name</p>
						</c:otherwise>
					</c:choose>
					<s:textfield name="personName" cssClass="span3 lookup-filterValue" id="businessContactPersonId" />
					...
					<p>Search using partial last name, first name or SSO</p>

...
<s:url var="exportURL" action="exportInvoiceFeeHistory" namespace="/int/admin" />
<s:url id="downloadFHSearchResultsURL" action="downloadFHSearchResults" namespace="/int/admin" />
				 	...
				 	<s:iterator value="apmDetails.feeHistoryDetails.fullSummaries" status="stat">
						<tr class="shown" id="paymentRow<s:property value="paymentId" />">
							...
							<td><s:if test="%{paymentID==null}">-</s:if><s:else>
								<s:form name="fhFeeSummaryForm" action="getFHFeeSummary" namespace="/int/apm" cssStyle="margin: 0px!important;">
									 ...
									 <s:hidden name="pageFrom" value="FeeHistory" />
	 								 <s:submit cssClass="submitLink" value="%{paymentID}" action="getFHFeeSummary" namespace="/int/apm" />
								</s:form></s:else>
							</td>
							<td>
							<s:form name="viewFullRequestForm" action="viewFullRequest" namespace="/int/apm" cssStyle="margin: 0px!important;">
								 <s:hidden name="requestId" value="%{ALOCRecordNumber}" />
								 <s:hidden name="returnToPage" value="FeeHistory" />
 								 <s:submit cssClass="submitLink" value="%{alocRecordId}" action="viewFullRequest" namespace="/int/apm" />
							</s:form>
							</td>
							<td><s:if test="%{bankReferenceNumber==null}">-</s:if><s:else><c:out value="${bankReferenceNumber}" /></s:else></td>
							<td class="hide GEApplicant"><s:if test="%{GEApplicant==null}">-</s:if><s:else><c:out value="${GEApplicant}" /></s:else></td>
							<td class="hide triPartyApplicant"><s:if test="%{triPartyApplicant==null}">-</s:if><s:else><c:out value="${triPartyApplicant}" /></s:else></td>
							<td class="hide beneficiaryName"><s:if test="%{beneficiaryName==null}">-</s:if><s:else><c:out value="${beneficiaryName}" /></s:else></td>
							<td><s:if test="%{paymentCurrencyName==null}">-</s:if><s:else><c:out value="${paymentCurrencyName}" /></s:else></td>
							...
							<td class="hide USDPaymentAmount"><s:if test="%{USDPaymentAmount==null}">-</s:if><s:else><c:out value="${USDPaymentAmount}" /></s:else></td>
							<td class="hide USFeeTotal"><s:if test="%{USFeeTotal==null}">-</s:if><s:else><c:out value="${USFeeTotal}" /></s:else></td>
							<td class="hide foreignFeeTotal"><s:if test="%{foreignFeeTotal==null}">-</s:if><s:else><c:out value="${foreignFeeTotal}" /></s:else></td>
							<td class="hide periodFeeCredits"><s:if test="%{periodFeeCredits==null}">-</s:if><s:else><c:out value="${periodFeeCredits}" /></s:else></td>
							<td class="hide periodAmendmentFees"><s:if test="%{periodAmendmentFees==null}">-</s:if><s:else><c:out value="${periodAmendmentFees}" /></s:else></td>
							<td class="hide otherFees"><s:if test="%{otherFees==null}">-</s:if><s:else><c:out value="${otherFees}" /></s:else></td>
							<td class="hide oneTimeFees"><s:if test="%{oneTimeFees==null}">-</s:if><s:else><c:out value="${oneTimeFees}" /></s:else></td>
							<td class="hide modelID"><s:if test="%{modelId==null}">-</s:if><s:else><c:out value="${modelId}" /></s:else></td>
							<td class="appPrnName"><s:if test="%{GEApplicant==null}">-</s:if><s:else><c:out value="${GEApplicant}" /></s:else></td>
							<td class="benOblName"><s:if test="%{beneficiaryName==null}">-</s:if><s:else><c:out value="${beneficiaryName}" /></s:else></td>
						</tr>
					</s:iterator>
				 </s:if>
				 <s:else>

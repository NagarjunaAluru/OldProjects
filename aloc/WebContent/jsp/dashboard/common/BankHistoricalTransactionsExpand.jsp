...
<div class="innerDiv">
 ...
 <s:form name="bankHistoricalTransExpForm" id="bankHistoricalTransExpFormID" action="openRequest" namespace="/int">                           
	    ...
	    <hwfs:checkComponentPermission name="NotBankBrokerReadOnly" domainName="BusinessAccess">
                	...
                	<c:if test="${instrumentTypeId eq 1 or instrumentTypeId eq 2 or instrumentTypeId eq 3}">
                		<c:if test="${state eq 'OUTSTANDING'}">
                			<s:submit key="label.dashboard.expandLabel.closeRequest" action="openRequest" namespace="/int" cssClass="expandRequestLink btn-secondary"/>
                		</c:if>
                	</c:if>
                <div class="clear"></div>

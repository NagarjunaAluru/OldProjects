...
<table id="tableBidProcessB" class="table table-striped sortable table-bordered paginate">
	...
	<thead>
		<tr id="column_head">
	        <th>
		        	<s:text name="label.dashboard.tableHeader.dateReceived"/>
	        </th>
	        ...
	        <c:choose>
        		<c:when test="${dashboardViewType eq 'BANKBIDPROCESS'}">
        			<th><s:text name="label.request.biddingBankName"/></th>
        		</c:when>
        		<c:otherwise>
        ...
        <tbody>
        	<s:if test="%{results.bankBidProcess == null || results.bankBidProcess.bidProcess.isEmpty()}">
        	...
        	<s:else>
        	<s:iterator value="results.bankBidProcess.bidProcess" status="stat">
        	
        	<tr class="shown">
                <td style="width: 100px;" id="${cc:getRowId(instrumentTypeId, instrumentPurposeId, bondTypeId, subBondTypeId)}">
                	...
                	<img alt="Loading..." class="dashboardExpandViewProcess" src="${pageContext.request.contextPath}/img/loading.gif" style="display:none;height: 20px;width: 20px; margin-left: 0px; margin-top: -10px;">
                </td>
                <td>
                ...
                <td>
                <s:form name="bidBankForm" id="bidBankFormID" action="openRequest" namespace="/int">
	                ...
	                <input type="hidden" name="bankCountryCd" value="${bankCountryCd}" class="bankCountryCd">
                ...
                <c:choose>
	        		<c:when test="${dashboardViewType eq 'BANKBIDPROCESS'}">
	        			<td>
                			...
                			<c:if test="${not empty bankName}"><c:out value="${bankName}"/></c:if>
              			</td>
	        		</c:when>
	        		<c:otherwise>
	        			<td>

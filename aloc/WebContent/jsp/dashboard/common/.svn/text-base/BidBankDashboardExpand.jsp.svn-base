...
<div class="innerDiv">
 ...
 <s:form name="bidBankExpForm" id="bidBankExpFormID" action="openRequest" namespace="/int">                           
    ...
    <c:if test="${empty bidFlag or (not empty bidFlag and bidFlag eq '')}">
	    <hwfs:checkComponentPermission name="NotBankBrokerReadOnly" domainName="BusinessAccess">
        	...
        	<s:hidden name="bankCountryName" value="%{bankCountryName}"/>
           	<s:submit key="label.dashboard.expandLabel.submitBidReply" action="openRequest" namespace="/int" cssClass="expandRequestLink btn-secondary"/>
         <div class="clear"></div>

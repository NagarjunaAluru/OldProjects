...
$(document).ready(function() {
	...
	$('#wantToContinue').click(function(e){
		if(continueAction == "affirm"){
			document.forms[0].action = contextURL + '/equityBusinessApprover.do?command=submitDealRequest&action='+continueAction;
	...
	$('#sendBack').click(function(e){
		...
		if(comments != ''){
			...
			document.forms[0].action = contextURL + '/equityBusinessApprover.do?command=submitDealRequest&action=sendBack';
			document.forms[0].submit();
		}else{
...
function saveData()
{
	...
	$('#cashManagementFBId').find('.radio-container').removeClass('conditional-radio-tri req-error');
	$('#actionButton').removeAttr('style');
	document.forms[0].action = contextURL + '/equityBusinessApprover.do?command=submitDealRequest&action=save';
	document.forms[0].submit();
}

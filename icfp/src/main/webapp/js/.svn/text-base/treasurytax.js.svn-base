...
$(document).ready(function() {
		...
		$('#submit').click(function(e){
			...
			if(action == undefined){
				$('#topErrorDiv').show();
				$('#updateStatusForm').find('.radio-container').addClass('conditional-radio-tri req-error');
				...
				$('#topErrorDiv').hide();
				$('#dCommentsError1').hide();
				$('#updateStatusForm').find('.radio-container').removeClass('conditional-radio-tri req-error');
		...
		$('#sendBack').click(function(e){
			...
			if(comments != ''){
				...
				document.forms[0].action = contextURL + '/treasuryTax/treasuryTax.do?command=submitDealRequest&action=sendBack';
...
function assignMember(){
	var teamMemberId = $('#teamMemberId').val();
	document.forms[0].action = contextURL + '/treasuryTax/treasuryTax.do?command=submitDealRequest&action=assignReview&teamMemberId='+teamMemberId;
	document.forms[0].submit();
}
...
function inputsCompleted()
{
	...
	$('#updateStatusForm').find('.radio-container').removeClass('conditional-radio-tri req-error');
	$('#actionButton').removeAttr('style');
	document.forms[0].action = contextURL + '/treasuryTax/treasuryTax.do?command=submitDealRequest&action='+action;
	document.forms[0].submit();
}
function saveData()
{
	...
	$('#updateStatusForm').find('.radio-container').removeClass('conditional-radio-tri req-error');
	$('#actionButton').removeAttr('style');
	document.forms[0].action = contextURL + '/treasuryTax/treasuryTax.do?command=submitDealRequest&action=save';
	document.forms[0].submit();
	}
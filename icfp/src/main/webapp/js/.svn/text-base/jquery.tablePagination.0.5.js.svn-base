 ...
 *      firstArrow - Image - Pass in an image to replace default image. Default: (new Image()).src="images/date-left.png"
 *      prevArrow - Image - Pass in an image to replace default image. Default: (new Image()).src="images/date-left.png"
 *      lastArrow - Image - Pass in an image to replace default image. Default: (new Image()).src="images/date-right.png"
 *      nextArrow - Image - Pass in an image to replace default image. Default: (new Image()).src="images/date-right.png"
 ...
 *      optionsForRows - Array - This is to set the values on the rows per page. Default: [10,25,50,100]
 ...
 *      dataToggle - String - Data Toggle value
 *
 *
 * @version 0.5
 ...
 */
 ...
 (function($){
	...
	$.fn.tablePagination = function(settings) {
		var defaults = {  
			firstArrow : (new Image()).src="../images/date-left.png",  
			prevArrow : (new Image()).src="../images/date-left.png",
			lastArrow : (new Image()).src="../images/date-right.png",
			nextArrow : (new Image()).src="../images/date-right.png",
			rowsPerPage : 10,
			currPage : 1,
			optionsForRows : [10,25,50,100],
			ignoreRows : [],
			topNav : false,
			dataToggle : new String("Show to side"),
			paginationId : new String("tablePagination"),
			paginationClass : new String("ledgend1")
		};  
      ...
      function createPaginationElements() {
        var htmlBuffer = [];
        htmlBuffer.push("<div id='"+defaults.paginationId+"' data-toggle='"+defaults.dataToggle+"' class='"+defaults.paginationClass+"'>");
        htmlBuffer.push("<span id='tablePagination_perPage'>");
        htmlBuffer.push("<select id='tablePagination_rowsPerPage' style='width: 50px; margin-top: 10px;'><option value='10'>10</option></select>");
        ...
        htmlBuffer.push("<input id='tablePagination_currPage' type='input' value='"+currPageNumber+"' size='1' style='width: 25px; margin-top: -10px;'>");

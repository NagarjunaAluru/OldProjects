...
function saveTreasurytax(obj){
	...
	if(validateFlag==false){
		if(actionType == 'saveNextLeg'){
			$('#saveNextLeg').show();
			$('#saveLeg').hide();
			document.forms[0].action = contextURL + '/treasuryTax/treasuryTaxLeg.do?command=saveNextLeg';
			document.forms[0].submit();
		}else if(actionType == 'saveReturnDeal'){
			$('#saveLeg').show();
			$('#saveNextLeg').hide();
			document.forms[0].action = contextURL + '/treasuryTax/treasuryTaxLeg.do?command=saveLeg';
			document.forms[0].submit();
		}
	}else{

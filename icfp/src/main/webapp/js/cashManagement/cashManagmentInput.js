...
function saveCashMgmt(obj){
	...
	if(validateFlag==false){
		if(actionType == 'saveNextLeg'){
			$('#saveNextLeg').show();
			$('#saveLeg').hide();
			document.forms[0].action = contextURL + '/cashManagement/cashManagementLeg.do?command=saveNextLeg';
			document.forms[0].submit();
		}else if(actionType == 'saveReturnDeal'){
			$('#saveLeg').show();
			$('#saveNextLeg').hide();
			document.forms[0].action = contextURL + '/cashManagement/cashManagementLeg.do?command=saveLeg';
			document.forms[0].submit();
		}
	}else{
...
$(document).ready(function() {
	$('#saveLeg1').click(function() {
		document.forms[0].action = contextURL + '/cashManagement/cashManagementLeg.do?command=saveLeg&Save=True';
		document.forms[0].submit();
	});

...
$(document).ready(function() {
	...
	$('#submit').click(function(e){
		...
		var changeApproval = $('input:radio[name=leBodGovernanceFlag]:checked').val();
		if(action == undefined){
			$('#topErrorDiv').show();
	...
	$('#sendBack').click(function(e){
		...
		if(comments != ''){
			...
			document.forms[0].action = contextURL + '/treasuryLegal/treasuryLegal.do?command=submitDealRequest&action=sendBack';
...
function inputsCompleted()
{
	...
	var action = $('input:radio[name=submitDealFB]:checked').val();
	var changeApproval = $('input:radio[name=leBodGovernanceFlag]:checked').val();
	if (changeApproval == undefined){
		$("#inputsCompletedModalID").modal('hide');
		$('#topErrorDiv').show();
		$('#cashManagementFBId').find('.span8').find('.radio-container').addClass('conditional-radio-tri req-error');
		$('#actionButton').removeAttr('style');
		...
		$('#cashManagementFBId').find('.radio-container').removeClass('conditional-radio-tri req-error');
		$('#actionButton').removeAttr('style');
		document.forms[0].action = contextURL + '/treasuryLegal/treasuryLegal.do?command=submitDealRequest&action='+action;
		document.forms[0].submit();
	}
}
...
function saveData()
{
	...
	$('#cashManagementFBId').find('.radio-container').removeClass('conditional-radio-tri req-error');
	$('#actionButton').removeAttr('style');
	document.forms[0].action = contextURL + '/treasuryLegal/treasuryLegal.do?command=submitDealRequest&action=save';
	document.forms[0].submit();
}

function assignMember(){
	var teamMemberId = $('#teamMemberId').val();
	document.forms[0].action = contextURL + '/treasuryLegal/treasuryLegal.do?command=submitDealRequest&action=assignReview&teamMemberId='+teamMemberId;
	document.forms[0].submit();
}
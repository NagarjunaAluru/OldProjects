...
$(document).ready(function() {
	...
	$('#sendBack').click(function(e){
		...
		if(comments != ''){
			...
			document.forms[0].action = contextURL + '/transferPricing/transferPricing.do?command=submitDealRequest&action=sendBack';
...
function inputsCompleted()
{
	...
	$('#cashManagementFBId').find('.radio-container').removeClass('conditional-radio-tri req-error');
	$('#actionButton').removeAttr('style');
	document.forms[0].action = contextURL + '/transferPricing/transferPricing.do?command=submitDealRequest&action='+action;
	document.forms[0].submit();
}
function saveData()
{
	...
	$('#cashManagementFBId').find('.radio-container').removeClass('conditional-radio-tri req-error');
	$('#actionButton').removeAttr('style');
	document.forms[0].action = contextURL + '/transferPricing/transferPricing.do?command=submitDealRequest&action=save';
	document.forms[0].submit();	
}
function assignMember(){
	var teamMemberId = $('#teamMemberId').val();
	document.forms[0].action = contextURL + '/transferPricing/transferPricing.do?command=submitDealRequest&action=assignReview&teamMemberId='+teamMemberId;
	document.forms[0].submit();
}
...
<c:choose>
			...
			<a href="${pageContext.request.contextPath}/frontoffice/RCALegRequest.do?command=redirectFundingRequest&isFrontOffice=Yes" class="right cancel single" style="margin-top:95px">Cancel</a>
			</c:when>
			...
			<c:when test="${actionId eq 31 || actionId eq 32 || actionId eq 33 || actionId eq 34 || actionId eq 35 || actionId eq 36 || actionId eq 37 || actionId eq 38 || actionId eq 39}">
						...
						<a href="${pageContext.request.contextPath}/${source}.do?command=load&DealRequestID=${sessionScope.deal.dealSeqId}" class="right cancel single" style="margin-top:95px">Cancel</a> 
			</c:when>

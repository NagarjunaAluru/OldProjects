...
<title>ICF | Treasury Legal</title>
<meta name="description" content="">
<meta name="author" content="">

<%@include file="../common/includeCssScripts.jsp" %>

<script src="${pageContext.request.contextPath}/js/treasurylegal.js" type="text/javascript"></script>
...
<%String legLenforJS ="0"; %>
	<% String sec = (String) request.getSession().getAttribute("section");//"myTasks";//myAssignedTasks
%>
<script type="text/javascript">
...
<body>
	<div class="container main">
		...
		<ul class="breadcrumb">
			...
			<li class="active">Treasury Legal</li>
		</ul>
		...
		<html:form action="/treasuryLegal/treasuryLegal.do" styleId="cashManagementFBId" method="post"   enctype="multipart/form-data">
		
		<h1 class="page-title span12">Treasury Legal</h1>
		...
		<c:choose>
			<c:when test="${not empty requestScope.isCPA && requestScope.isCPA == true}">
				...
				<input type="hidden" id="isCPA" name="isCPA" value="CPA" />
			</c:when>
			<c:otherwise>
				<input type="hidden" id="isCPA"  name="isCPA"  value="NonCPA" />
		...
		<c:set var="nonStandardDocsFlag" value="${deal:getNonStandardDocsFlag(pageContext.request)}" />	
			...
			<%@ include file="../common/inc/editAdditionalApprovers.jsp"%>
			...
			<jsp:include page="../common/inc/commentsLog.jsp">
				...
				<jsp:param value="treasuryLegal/treasuryLegal" name="path"/>
				<jsp:param value="Treasury Legal" name="name"/>
			...
			<div class="span3 right submit-box">
				<div class="form-row">
					...
					<div class="radio-container">
						<c:if test="${sessionScope.section eq 'myTasks'}">
							<label class="radio" id="affirmButton">
								<input type="radio" value="affirm" name="submitDealFB">
								Inputs Completed
							</label>
	
							<label class="radio" id="sendBackButton">

...
%>
		...
		<div class="form-mod"  style="overflow-x: auto;">
			...
			<h2 class="span12 collapsible">Exceptions - ${fn:length(legSummaryVO.exceptions)}</h2>
			<div class="clear"></div>
			<div class="row">
				<div class="span12">
				 <table class="table table-striped table-bordered sortable exceptions">
					...
					<tbody>
				<%int k=0; %>
					<c:forEach var="exceptionRequest" items="${legSummaryVO.exceptions}" varStatus="itemNo">
					
						
						<tr>
							<td>${legSummaryVO.legSeqId}</td>
							<td><p><b>Term &amp; Conditions</b><br />
								<c:if test="${empty exceptionRequest.standardTermsConditions}">
									--
								</c:if>
								<c:if test="${not empty exceptionRequest.standardTermsConditions}">
							...
							<td><p><b>Estimated Timeframe</b><br />
								<c:if test="${empty exceptionRequest.remediationTimeline}">
									--
								</c:if>
								<c:if test="${not empty exceptionRequest.remediationTimeline}">
							...
							<td><p><b>Attachments</b><br />
								<c:set var="exptIndex" value="<%=String.valueOf(++k)%>" />
					<%

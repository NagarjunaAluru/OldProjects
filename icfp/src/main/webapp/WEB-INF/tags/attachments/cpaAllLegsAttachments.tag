...
<div class="row">
	<div class="span12">
	 	<table class="table table-striped table-bordered no-bottom">
			...
			<tbody>
				<c:set var="attachmentLegs" value="${atmtfunctions:getCPALegSummaryVOs(pageContext.request)}"/>
				<c:if test="${not empty attachmentLegs}">
				...
				<c:if test="${legalAgAtmtPermissions.viewable}">
					<tr>
						...
						<c:choose>
							<c:when test="${not empty legIndex}">
								<td>
...
<%-- Journal Entries --%>
				<c:set var="journalAtmtPermissions" value="${atmtfunctions:getAttachmentPermissions(7, pageContext.request)}" />
					...
					<tr>
						...
						<c:choose>
							<c:when test="${not empty legIndex}">
								...
								<td>
									<c:choose>
										<c:when test="${((not empty mode) && mode == 'edit' && otherAtmtPermissions.editable) or (otherAtmtPermissions.dealPermissions.closed)}">
